...
export class DraftModifyComponent implements OnInit {
    ...
    handleKeydown(event: KeyboardEvent, playerIndex: number, roundIndex: number, pickIndex: number) {
    ...
    focusNextInput(playerIndex: number, roundIndex: number, pickIndex: number) {
        ...
        if (pickIndex < maxPicks) {
            const nextInput = document.getElementById(`cardNumber-${playerIndex}-${roundIndex}-${pickIndex + 1}`);
            if (nextInput) {
                nextInput.focus();
        ...
        if (roundIndex < 2) {
            const nextInput = document.getElementById(`cardNumber-${playerIndex}-${roundIndex + 1}-0`);
            if (nextInput) {
                nextInput.focus();
                (nextInput as HTMLInputElement).select();
                return;
    ...
    focusPreviousInput(playerIndex: number, roundIndex: number, pickIndex: number) {
        ...
        if (pickIndex > 0) {
            const prevInput = document.getElementById(`cardNumber-${playerIndex}-${roundIndex}-${pickIndex - 1}`);
            if (prevInput) {
                prevInput.focus();
        ...
        if (roundIndex > 0) {
            const maxPicks = this.draftDef.cardsPerPack - 1;
            const prevInput = document.getElementById(`cardNumber-${playerIndex}-${roundIndex - 1}-${maxPicks}`);
            if (prevInput) {
                prevInput.focus();
                (prevInput as HTMLInputElement).setSelectionRange(
                    (prevInput as HTMLInputElement).value.length,
        ...
        if (playerIndex > 0) {
            const maxPicks = this.draftDef.cardsPerPack - 1;
            const prevInput = document.getElementById(`cardNumber-${playerIndex - 1}-2-${maxPicks}`);
            if (prevInput) {
                prevInput.focus();
                (prevInput as HTMLInputElement).setSelectionRange(
                    (prevInput as HTMLInputElement).value.length,
        ...
        this.draftDef.playerPicks.forEach(player => {
            player.rounds.forEach(round => {
                if (round.picks) {
                    ...
                    while (round.picks.length > 0 && !round.picks[round.picks.length - 1]?.cardNumber) {
                        round.picks.pop();
                    }
                    // Filter out completely empty picks
                    round.picks = round.picks.filter(pick => pick.cardNumber && pick.cardNumber.trim() !== '');
                }
            });
        });
