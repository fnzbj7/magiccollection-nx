...
export class DraftViewerCoreComponent implements OnInit, AfterViewChecked {
    ...
    onFilterChange() {
        ...
        const cardPick =
            round.picks && Array.isArray(round.picks) ? round.picks[+this.pickSelect] : null;
        ...
        if (cardPick && cardPick.cardNumber) {
            ...
            const maxPicks = Math.max(
                ...this.draftDef.playerPicks.map(p => {
            ...
            for (let pick = +this.pickSelect; pick < maxPicks; pick++) {
                for (let person = 0; person < this.draftDef.playerPicks.length; person++) {
                    ...
                    if (
                        roundData.picks &&
                        Array.isArray(roundData.picks) &&
                        roundData.picks[pick]
                    ) {
                    ...
                    if (cardAtPosition && cardAtPosition.cardNumber) {
                        const cardIdentifier =
                            cardAtPosition.setCode + cardAtPosition.cardNumber.padStart(3, '0');
                        ...
                        if (person === +this.playerSelect && pick === +this.pickSelect) {
                            ...
                            for (let i = 0; i < reconstructedPack.length; i++) {
                                const packCardIdentifier =
                                    reconstructedPack[i].setCode +
                                    reconstructedPack[i].cardNumber.padStart(3, '0');
